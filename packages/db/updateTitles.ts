...
async function main() {
  ...
  for (const u of introUpdates) {
    await prisma.unit.updateMany({
      where: { stageId: stage.id, order: u.order },
      data: { title: u.title, description: u.description }
    });
    console.log(`  ✓ [Introduction to Java] Unit ${u.order}: "${u.title}"`);
  }

  // --- Java Fundamentals ---
  const fundamentalsStage = await prisma.stage.findFirst({
    where: { title: "Java Fundamentals" }
  });

  if (fundamentalsStage) {
    const fundamentalsUpdates = [
      ...
      { order: 12, title: "Pass By Value",                 description: "Understand how Java passes arguments by value, including references." },
    ];

    for (const u of fundamentalsUpdates) {
      await prisma.unit.updateMany({
        where: { stageId: fundamentalsStage.id, order: u.order },
        data: { title: u.title, description: u.description }
      });
      console.log(`  ✓ [Java Fundamentals] Unit ${u.order}: "${u.title}"`);
    }
  }
